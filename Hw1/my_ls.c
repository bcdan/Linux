...
struct fileInfo{ // acts as an simpler and thinner i-node
    char permissions[11]; // 10 chars + /0 -> acts as a string toribe a file's permission
    char* groupId;
    char* userID;
...
struct fileInfo constructInfo(const char* name){
    ...
    info.groupId=grp->gr_name;
    info.numOfLinks=sb.st_nlink;
...
void printInfo(struct fileInfo* info,const char* name){
    printf("%2s ",info->permissions);
    printf("%5d ", info->numOfLinks);
    printf("%5s %5s ",info->userID,info->groupId);
    printf("%10ld ", info->size);
    printf("%2s %2d %2d %2.2d:%.2d ",Months[info->month], info->day,info->year, info->hour, info->min);

    if(info->permissions[0]=='l'){ //if a link
        ssize_t size;
    ...
    else if(info->permissions[9]=='x' && info->permissions[0]!='d') //if executable
        printf(GRN"%s\n"RESET, name);
...
void permissionsToString(struct fileInfo* info){
    int i = 0;
    info->permissions[i++]=S_ISDIR(info->st_mode)?'d':
                                S_ISFIFO(info->st_mode)?'p':
                                S_ISLNK(info->st_mode)?'l':'-';
    info->permissions[i++]=(S_IRUSR & info->st_mode) ? 'r' : '-';
    info->permissions[i++]=(S_IWUSR & info->st_mode) ? 'w' : '-';
    info->permissions[i++]=(S_IXUSR & info->st_mode) ? 'x' : '-';
    info->permissions[i++]=(S_IRGRP & info->st_mode) ? 'r' : '-';
    info->permissions[i++]=(S_IWGRP & info->st_mode) ? 'w' : '-';
    info->permissions[i++]=(S_IXGRP & info->st_mode) ? 'x' : '-';
    info->permissions[i++]=(S_IROTH & info->st_mode) ? 'r' : '-';
    info->permissions[i++]=(S_IWOTH & info->st_mode ) ? 'w' : '-';
    info->permissions[i++]=(S_IXOTH & info->st_mode) ? 'x' : '-';
    info->permissions[i]='\0';
